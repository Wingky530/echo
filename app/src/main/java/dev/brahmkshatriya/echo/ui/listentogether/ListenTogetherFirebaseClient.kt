...
// UPDATE: Added queueContext to hold multiple track IDs
data class WsMessage(
    ...
    val queueContext: String? = null // Format: "id1,id2,id3..."
)
...
class ListenTogetherFirebaseClient {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {                                              
                ...
                trySend(
                    WsMessage(
                        ...
                        snapshot.child("queueContext").value?.toString() // Extract queue context
                    )
                )
            }                                                                                                
    ...
    fun send(code: String, msg: WsMessage, isHost: Boolean = false) {
        if (msg.type == "SYNC") {
            ...
            // Only send queueContext if it's not null to save bandwidth
    ...
    fun getCurrentState(code: String, onResult: (WsMessage?) -> Unit) {                                      
        db.getReference("sessions/$code/state").get().addOnSuccessListener { s ->                            
            if (!s.exists()) onResult(null) else onResult(
                WsMessage(
                    "SYNC", 
                    s.child("trackId").value?.toString(), 
                    s.child("extensionId").value?.toString(), 
                    s.child("positionMs").value?.toString()?.toLongOrNull() ?: 0L,
                    s.child("isPlaying").value?.toString()?.toBoolean() ?: false, 
                    s.child("senderId").value?.toString() ?: "", 
                    null, 
                    null, 
                    s.child("timestamp").value?.toString()?.toLongOrNull() ?: 0L, 
                    s.child("trackTitle").value?.toString(),
                    s.child("queueContext").value?.toString() // Extract queue context
                )
            )
